...
| **Test information in doc** | `targeted_verification` — a list of test names taken from a static test inventory | **None.** Workflow 3 dynamically selects tests at test time. No test names stored in the doc. |
| **Test inventory** | `docs/test_inventory.json` refreshed nightly by a project-level testing workflow; a per-component testing workflow enumerated tests for one component | **None.** No static test inventory. Workflow 3 selects tests dynamically. |
| **Freshness model** | `last_validated` commit+date, compared against the latest merged PR in an explicitly stored `validation_group` field (change-aware) | `last_validated` validation hash **stamped exclusively by Workflow 4**, compared against the latest commit touching the owning component or any component in `Affects ∪ Affected By` (change-aware, validation group implicit, commit-hash based via a GitHub Action) |
| **Impact discovery mechanism** | Mutation testing (intentional break + observe downstream test failures); tests enumerated statically and run directly | Mutation testing (intentional break + delegate to AI-driven testing); Workflow 3 dynamically selects and runs appropriate tests |
| **Bootstrap** | Workflow 1: nested-loop mutation testing across all N components, with static test enumeration | Workflow 1 (Full-System Scan): nested-loop mutation testing across all N components, delegates testing of candidate-affected components to Workflow 3 via parallel sub-agents |
| **Incremental maintenance** | Workflow 2: single-pass mutation testing of new component against existing components, with static test enumeration | Workflow 2 (Single-Component Scan): the same mutation + test-all algorithm as Workflow 1 **minus the outer loop** — it works on exactly one component you hand it, tests every other component in the system, and writes the component's MD file. If an MD already exists under that component's name, the scan result **automatically replaces** it (previous version preserved in git history); otherwise W2 creates one. No mode flags. Delegates testing to Workflow 3 via parallel sub-agents, and triggers Workflow 4 at the end. |
| **Testing workflow** | A project-level testing workflow enumerated the test inventory; a per-component testing workflow enumerated tests for one component | Workflow 3 (AI-Driven Testing) — given a (component, mutation context) task, dynamically determines and executes appropriate tests. Inputs/outputs specified; internals deliberately under-specified. |
| **Edit-and-verify loop** | Not specified — left to the orchestration layer (AGENTS.md) | **Workflow 5 (Edit Component Loop)** — take edit prompt, back up source, apply edit, fan out W3 sub-agents against every component under `Affects:`, cancel-on-first-failure, restore-and-retry until safe or max-retries hit. **Workflow 5 has no relation to Workflow 2** — it never invokes it, not even after a safe edit; doc refresh after edits is left to the CI staleness check, which triggers W2 independently when the hash comparison shows the doc is outdated. |
| **New-component bootstrap** | Not specified — implicit in Workflow 2 | **Workflow 6 (Create New Component)** — take creation prompt, generate source code, write to disk, invoke Workflow 2 on the new component — no MD exists under its name yet, so W2 creates one (it would automatically replace it if one existed). W2 internally triggers W4. One-shot new-component creation + documentation. |
...
| **Parallelism** | Not specified — workflows were assumed sequential | Explicit sub-agent fan-out model in Workflows 1, 2, and 5; testing tasks dispatched to parallel sub-agents that each invoke Workflow 3 |
| **Lifecycle** | Full: Creation → Documentation → Testing → Impact Discovery → Maintenance → Revalidation | Full: Creation → Single-Component Scan → AI-Driven Testing → Affects Discovered → Reverse Sync → Steady State |
| **CI role** | Emit change-aware staleness warnings; trigger Workflow 2 on staleness; no time-based fail | Same change-aware model, but hash-based: a GitHub Action looks at the hash in each MD file (stamped by Workflow 4) and compares it against the latest commit made on the component that owns the file, or on any component in its `Affects` / `Affected By` lists, and triggers Workflow 2 on staleness |
...
| **Confidence in the proposal** | Medium — workflows harder to evaluate without a pilot; accuracy deterministic (stored test lists) but stale-prone | Lower — adds a new assumption (AI can dynamically select appropriate tests) that the prior form did not make. The trade is explicit; the empirical question is flagged for pilot. |

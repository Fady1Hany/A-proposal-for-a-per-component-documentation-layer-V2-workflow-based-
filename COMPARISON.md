...
|---|---|---|
| **Field ownership** | Split: auto-generated structural fields (`location`, `depends_on`, `used_by`) vs hand-authored intent fields (`change_impact`, `targeted_verification`, `verified_against`) | All workflow-produced; humans review and edit after. The `Affected By` field is exclusively produced by Workflow 4. |
| **Primary relationship fields** | `depends_on` (imports) + `used_by` (importers) + `change_impact` (hand-authored impact description)| `Affects` (mutation-derived) + `Affected By` (reverse-synced by Workflow 4). `depends_on`/`used_by`/`change_impact` are removed. |
...
| **update md files** | 90 days from `verified_against` date  | `last_validated` validation hash **stamped exclusively by Workflow 4**, compared against the latest commit touching the owning component or any component in `Affects ∪ Affected By` (change-aware, validation group implicit, commit-hash based via a GitHub Action) |
...
| **Schema fields** | `component`, `location`, `verified_against`, `depends_on`, `used_by`, `change_impact`, `targeted_verification`, plus optional `public_interface`, `stability`, `notes` | `component`, `location`, `last_validated`, `affects`, `affected_by`, `provenance`, plus optional `public_interface`, `stability`, `notes`, `spec_version`. Removed: `depends_on`, `used_by`, `change_impact`, `targeted_verification`, `validation_group`. |

...
There is a second reason this proposal exists now, and it comes from a companion repo of the author's essays, [Thoughts-about-LLM](https://github.com/Fady1Hany/Thoughts-about-llm). Those essays walk the energy question behind AI from the supply side — compact reactors, nuclear thermodynamics, the ATP economy of the human body — and arrive at an inversion: if energy generation is locked, the other road is to *need less*. Same capability, a fraction of the compute. The essays stop at that door; this proposal is an attempt to open it in one domain — software engineering — and to measure what comes out.

The author's position, stated as a belief rather than an established fact: **in software engineering, the context window we already have is enough.** No realistic window will ever hold a 10,000-component codebase, and a window ten times today's size would only postpone the same failure — the agent would still spend its budget re-reading structure it cannot act on. What the agent actually needs is a few hundred *right* tokens: which components break if this one changes. That is a bounded amount of information, and it fits comfortably inside the ~1M-token windows that current models already ship. The per-component doc layer is the compression mechanism that keeps an entire codebase's behavioral structure within that budget — it is how the existing window is used at maximum efficiency instead of being treated as a limitation to engineer around. In the essays' vocabulary, the bet is that the next gains come from *architecture* — engineering the environment the model operates in — rather than from scale: an architecture for artificial intelligence, not just a larger LLM.

The energy framing has also shifted since the essays were written, in this proposal's favor. Running these models is no longer the hard part — inference efficiency per watt keeps improving (the essays document this hardware trend directly), and capable models are increasingly available to everyone, in many cases for free. What remains expensive is *creating* ever-larger frontiers. A design that extracts more software-engineering capability from the current generation of models leans less on that frontier — which is precisely the demand-side move the essays call for. Whether that bet generalizes beyond software engineering is an open question, and the author does not claim it as fact; the pilot described in [`COST.md`](./COST.md) is where it can start to be measured.

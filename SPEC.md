...
6. **The orchestration layer** (e.g. `AGENTS.md`) defines which workflow runs when. The default triggers:
   ...
   - Workflow 4 (Reverse Relationship Sync): invoked by Workflows 1, 2, and 6 after they complete — always the same full nested loop over every MD file in the project, no diff input. Workflow 4 also stamps the validation hash (`last_validated`) into the MD file of every component; it is the sole writer of that field. Workflow 5 does not invoke Workflow 4.

...
## Relationship to the author's Thoughts-about-LLM essays

The author keeps a companion repo of essays — [Thoughts-about-LLM](https://github.com/Fady1Hany/Thoughts-about-llm) — that works the energy question behind ever-larger models from the supply side: compact reactors, nuclear thermodynamics, the ATP economy of the human body. The essays end by inverting the problem: if energy generation is locked, the other road is to *need less* — the same capability at a fraction of the compute.

This proposal is that inversion applied to one domain. The author's belief — stated as a belief, not an established fact — is that **software engineering does not need a bigger context window**. No window will ever hold a whole codebase, and a larger one only postpones the same failure: the agent would still spend its budget re-reading structure it cannot act on. What an agent needs is the *right* few hundred tokens — who breaks if I change this — which fits comfortably inside the ~1M-token windows current models already provide. The per-component doc layer is the compression mechanism that makes maximum use of the window we already own, moving current LLMs from solving the problem of *coding* toward the actual problem of *software engineering*. The energy picture has shifted the same way: running these models is now cheap and increasingly available to everyone for free, while *creating* ever-larger ones stays expensive — so a design that needs less model leans less on that frontier. In the essays' terms, this is a bet on **architecture over scale**. Full argument in [`RATIONALE.md` §Part 8](./RATIONALE.md).
